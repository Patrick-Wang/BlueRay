...
public class SaleDaoImpl implements SaleDao {
	...
	public List<HTXX> getSaleData(String approveType, String approved,
			Integer pagesize, Integer pagenum, Integer pagecount, Integer col,
			Boolean sort) {
		...
		if (null != colName) {
			...
			sqlWhere += " h." + fields[col].getName() + "=cls." + id;
			sqlOrder += "cls." + colName + (sort ? " asc" : " desc");
		} else {
			if (col >= 0) {
				sqlOrder += " h." + fields[col].getName()
						+ (sort ? " asc" : " desc");
			}
		}
		...
		try {
			if (field.getType().getSimpleName()
				...
				try {
					...
					.equals(Date.class.getSimpleName())) {
				try {
	...
	public List<HTXX> getSearchedSaleData(String approveType, String approved,
		...
		if (null != colName) {
			Class<?> cls = HTXX.getFroeignClass(col);
			sqlOrder += cls.getSimpleName() + "_." + colName
					+ (sort ? " asc" : " desc");
		} else {
			if (col >= 0) {
				sqlOrder += " h."
						+ HTXX.class.getDeclaredFields()[col].getName()
						+ (sort ? " asc" : " desc");
			}
		}
		...
		for (int i = 1; i < fields.length - 1; ++i) {
			if (i != col) {
				...
				if (null != cls) {
					...
					sqlWhere += " h." + fields[i].getName() + "="
							+ cls.getSimpleName() + "_." + getForginId(cls)
							+ " ";
				}
			}
		}
		...
		for (int i = 1; i < fields.length - 1; ++i) {
			...
			if (null != cls) {
				...
				sqlWhere += cls.getSimpleName() + "_." + getForginName(cls)
						+ "='" + keyword + "' ";
			} else {
				try {
					...
					if ("is date".equals(e.getMessage())) {
						...
						sqlWhere += "h." + fields[i].getName() + "= :data" + i
								+ " ";
						paramMap.put("data" + i, Date.valueOf(keyword));
					}
				}
	...
	public List<HTXX> getSearchedSaleData(String approveType, String approved,
			Integer pagesize, Integer pagenum, Integer pagecount, Integer col,
			Boolean sort, JSONArray keyWords) {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		String sql = "select h from HTXX h, KHXX KHXX_, CPGGXHXX CPGGXHXX_, ZCXX ZCXX_, ZDQDYFLXX ZDQDYFLXX_, YYLGGFLXX YYLGGFLXX_, BPQXHFLXX BPQXHFLXX_, BMQXHFLXX BMQXHFLXX_ ";
		String colName = null;
		String id = null;
		...
		for (int i = 1; i < keyWords.size() + 1; ++i) {
			String keyword = keyWords.getString(i - 1);
			if (!keyword.isEmpty()) {
				Class<?> cls = HTXX.getFroeignClass(i);
				if (null != cls) {
					if (!firstAnd) {
					...
					try {
						...
						if ("is date".equals(e.getMessage())) {
							...
							sqlWhere += "h." + fields[i].getName() + "= :data"
									+ i + " ";
							paramMap.put("data" + i, Date.valueOf(keyword));
						}
					}
