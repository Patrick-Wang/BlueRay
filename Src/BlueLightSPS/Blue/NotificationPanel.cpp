...
void initMap(){
		...
		NOTIFICATION_SALE_MAP(notificationSaleMap, khmc);	//瀹㈡埛鍚嶇О"
		NOTIFICATION_SALE_MAP(notificationSaleMap, ggxh);	//瑙勬牸鍨嬪彿"
		NOTIFICATION_SALE_MAP(notificationSaleMap, sl);	//鏁伴噺"
		NOTIFICATION_SALE_MAP(notificationSaleMap, zc);	//杞存壙"
		NOTIFICATION_SALE_MAP(notificationSaleMap, dfr);	//鍗曞缁?
		NOTIFICATION_SALE_MAP(notificationSaleMap, zdqdy);	//鍒跺姩鍣ㄧ數鍘?
		NOTIFICATION_SALE_MAP(notificationSaleMap, yylgg);	//鏇冲紩杞鏍?
		NOTIFICATION_SALE_MAP(notificationSaleMap, jf);	//鏈烘埧"
		NOTIFICATION_SALE_MAP(notificationSaleMap, bpqxh);	//鍙橀鍣ㄥ瀷鍙?
		NOTIFICATION_SALE_MAP(notificationSaleMap, bmqxh);	//缂栫爜鍣ㄥ瀷鍙?
		NOTIFICATION_SALE_MAP(notificationSaleMap, dlcd);	//鐢电紗闀垮害"
		NOTIFICATION_SALE_MAP(notificationSaleMap, zxcd);	//闂哥嚎闀垮害"
		NOTIFICATION_SALE_MAP(notificationSaleMap, mpzl);	//閾墝绛夎祫鏂?
		NOTIFICATION_SALE_MAP(notificationSaleMap, bz);	//澶囨敞"
		NOTIFICATION_SALE_MAP(notificationSaleMap, ddrq);	//璁㈠崟鏃ユ湡"
		NOTIFICATION_SALE_MAP(notificationSaleMap, zjdy);	//涓绘満鐢靛帇
		NOTIFICATION_SALE_MAP(notificationSaleMap, zjys);	//涓绘満棰滆壊
		NOTIFICATION_SALE_MAP(notificationSaleMap, zdqxh);	//鍒跺姩鍣ㄥ瀷鍙?
		NOTIFICATION_SALE_MAP(notificationSaleMap, zyz);	// 宸?鍙崇疆
		NOTIFICATION_SALE_MAP(notificationSaleMap, bzxdtgg);// 鍖呰绠?搴曟墭瑙勬牸
		NOTIFICATION_SALE_MAP(notificationSaleMap, gh);	// 宸ュ彿
		NOTIFICATION_SALE_MAP(notificationSaleMap, zzs);	// 鍒堕�犲晢
		NOTIFICATION_SALE_MAP(notificationSaleMap, khqy);	// 瀹㈡埛鍖哄煙
		NOTIFICATION_SALE_MAP(notificationSaleMap, yxj);	//浼樺厛绾?
		NOTIFICATION_SALE_MAP(notificationSaleMap, ywsh);	//瀹℃牳-涓氬姟
		NOTIFICATION_SALE_MAP(notificationSaleMap, jhsh);	//瀹℃牳-璁″垝"



		NOTIFICATION_PLAN_MAP(notificationPlanMap, hth);	//鍚堝悓鍙?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, khmc);	//瀹㈡埛鍚嶇О"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, ggxh);	//瑙勬牸鍨嬪彿"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, cg);
		NOTIFICATION_PLAN_MAP(notificationPlanMap, sl);	//鏁伴噺"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zc);	//杞存壙"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, dfr);	//鍗曞缁?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zdqdy);	//鍒跺姩鍣ㄧ數鍘?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, yylgg);	//鏇冲紩杞鏍?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, jf);	//鏈烘埧"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bpqxh);	//鍙橀鍣ㄥ瀷鍙?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bmqxh);	//缂栫爜鍣ㄥ瀷鍙?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, dlcd);	//鐢电紗闀垮害"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zxcd);	//闂哥嚎闀垮害"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, mpzl);	//閾墝绛夎祫鏂?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bz);	//澶囨敞"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, ddrq);	//璁㈠崟鏃ユ湡"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zjdy);	//涓绘満鐢靛帇
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zjys);	//涓绘満棰滆壊
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zdqxh);	//鍒跺姩鍣ㄥ瀷鍙?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zyz);	// 宸?鍙崇疆
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bzxdtgg);// 鍖呰绠?搴曟墭瑙勬牸
		NOTIFICATION_PLAN_MAP(notificationPlanMap, gh);	// 宸ュ彿
		NOTIFICATION_PLAN_MAP(notificationPlanMap, zzs);	// 鍒堕�犲晢
		NOTIFICATION_PLAN_MAP(notificationPlanMap, khqy);	// 瀹㈡埛鍖哄煙
		NOTIFICATION_PLAN_MAP(notificationPlanMap, yxj);	//浼樺厛绾?
		NOTIFICATION_PLAN_MAP(notificationPlanMap, scrq);	//鐢熶骇鏃ユ湡"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, jhshyw);	//璁″垝瀹℃牳-涓氬姟"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, jhshjh);	//璁″垝瀹℃牳-璁″垝"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bzrq);	//鍖呰鏃ユ湡"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bzshyw);	//鍖呰瀹℃牳-涓氬姟"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, bzshjh);	//鍖呰瀹℃牳-璁″垝"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, fhrq);	//鍙戣揣鏃ユ湡"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, tcbh);	//鎶曚骇缂栧彿"
		NOTIFICATION_PLAN_MAP(notificationPlanMap, ccbh);	//鍑哄巶缂栧彿"
}
...
void CNotificationPanel::OnInitChilds()
{
	...
	else
	{
		...
		m_btnApproveInSecond.SetWindowText(_T("审核"));
...
void CNotificationPanel::OnBnClickedBtnApprove()
{
	...
	case CNotificationPanel::Approving_PlanBZRQPlan:
	{
		...
		CNotificationAddDlgForPlan dlg(_T("计划审核"));
		...
		if (bIfUpdateSaleInfo)
		{
			...
			public:
				...
				virtual void OnFailed(){
					m_notificationPanel.MessageBox(_T("网络错误"), _T("警告"), MB_OK | MB_ICONWARNING);
					m_notificationPanel.GetParent()->EnableWindow(TRUE);
				}
			};
