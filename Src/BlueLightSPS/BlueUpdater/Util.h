...
namespace Util_Tools
{
	...
	public:
		...
		static void base64_decode(CString& strBase64Src, CString& strBase64Dest);
		static CString base64_decode(CString& strBase64Src);
		...
		static void Utf16leToANSI(const CString& unicodeString, std::string& str)
		{
			int stringLength = ::WideCharToMultiByte(CP_ACP, NULL, unicodeString, wcslen(unicodeString), NULL, 0, NULL, NULL);
			char* buffer = new char[stringLength + 1];
			::WideCharToMultiByte(CP_ACP, NULL, unicodeString, wcslen(unicodeString), buffer, stringLength, NULL, NULL);
			buffer[stringLength] = '\0';
			str = buffer;
			delete[] buffer;
		}

		static void ANSIToUtf16LE(std::string& str, CString& unicodeString)
		{
			int stringLength = ::MultiByteToWideChar(CP_ACP, NULL, str.c_str(), str.length(), NULL, 0);
			TCHAR* buffer = new TCHAR[stringLength + 1];
			::MultiByteToWideChar(CP_ACP, NULL, str.c_str(), str.length(), buffer, stringLength);
